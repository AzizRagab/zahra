#!/usr/bin/env bash
# ZAHRA workspace — full installer for Kali Linux.
# Installs all three services: zahra (Python), Pentest-Swarm-AI (Go,
# plus the recon toolchain), OmniRoute (Node/TypeScript). Each is a
# separate process — see start-all.sh to run them together.
set -euo pipefail
...
echo "============================================================"
echo "  ZAHRA workspace — Kali Linux full installer"
echo "  1) zahra (Python)  2) Pentest-Swarm-AI (Go + recon tools)  3) OmniRoute (Node)"
echo "============================================================"
...
# ---- 2) Pentest-Swarm-AI (+ recon toolchain) ---------------------------
info "[2/3] Installing Pentest-Swarm-AI (Go) + recon toolchain..."
if [ -x "$ROOT/services/pentest-swarm/install-kali.sh" ]; then
    "$ROOT/services/pentest-swarm/install-kali.sh"
else
    fail "services/pentest-swarm/install-kali.sh not found"
fi
...
if command -v node >/dev/null 2>&1; then
    ...
    if [ "$CUR_NODE_MAJOR" -ge "$REQUIRED_NODE_MAJOR_MIN" ]; then
        ...
        warn "Node $(node -v) found (Kali's apt Node is often older) but OmniRoute requires >=22.22.2 <23 or >=24"
    fi
fi
